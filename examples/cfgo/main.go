...
func main() {
	...
	echKeySetBytes, err := echKeySet.MarshalBinary()
	if err != nil {
		panic(err)
	}

	echKeys, err := tls.EXP_UnmarshalECHKeys(echKeySetBytes)
	if err != nil {
		panic(err)
	}

	tlsECHKeySet, err := tls.EXP_NewECHKeySet(echKeys)
	if err != nil {
		panic(err)
	}
	...
	tlsECHConfigList, err := tls.UnmarshalECHConfigs(echConfigListBytes)
	if err != nil {
		panic(err)
	}
	...
	clientTLSConn := tls.Client(clientConn, &tls.Config{
		ECHEnabled:         true,
		ClientECHConfigs:   tlsECHConfigList,
		ServerName:         innerSNI,
	...
	serverTLSConn := tls.Server(serverConn, &tls.Config{
		ECHEnabled:        true,
		ServerECHProvider: tlsECHKeySet,
